...
export const Checkout = () => {
    ...
    return (
        ...
        {cart.length === 0 
            ...
            : // Formulario
            <div className="rounded shadow-xl w-3/4 md:w-1/2 mx-auto m-4 p-4 border-2">
                ...
                <hr/>
                    <form className="pt-5 space-y-4" onSubmit={handleSubmit}>
                        <div>
                            <label>Nombre {(values.name.length !== 0 && values.name.length < 3) && <span className="text-gray-500  text-sm">debe contener más de 3 letras</span>}</label>
                            <input 
                        ...
                        <div>
                            <label>Apellido {(values.lastName.length !== 0 && values.lastName.length < 4) && <span className="text-gray-500  text-sm">debe contener más de 3 letras</span>}</label>
                            <input 
                                className={inputStyle} 
                                type="text" 
                        ...
                        <div>
                            <label>Teléfono {(values.phone.length !== 0 && values.phone.length !== 10) && <span className="text-gray-500  text-sm">debe contener 10 números</span>}</label>
                            <input 
                                className={inputStyle} 
                        ...
                        <div>
                            <label>Repetí tu teléfono {(values.phone !== values.phone2) && <span className="text-gray-500  text-sm">debe coincidir con el campo anterior!</span>}</label>
                            <input 
                                className={inputStyle} 
                                placeholder="Sólo números. Ej: 1190908080"
                                type="number" 
